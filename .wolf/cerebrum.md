...
- **STATUS.md can drift from real file state** if changes happen without a matching update pass (e.g. Galerie was fully renamed to Projekte — `views/Galerie/`, `ui/GalleryPopup.tsx`, `GalleryItem` all replaced by `views/Projekte/`, `ui/ProjektPopup.tsx`, `ProjektItem` — before that was reflected in STATUS.md). When a user references a file/path that doesn't match what STATUS.md describes, trust the actual repo (Glob/Grep) over the doc, then reconcile STATUS.md to match reality rather than assuming the doc is current.
